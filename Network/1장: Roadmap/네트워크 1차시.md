...
- Enterprise access network(Ethernet)
  ...
  - 이 라우터는 `ISP(Internet Service Provider)`가 준 링크를 통해 ISP의 라우터에 **직접** 연결된다.
...
- Circuit Switching(회선 교환 방식)
  ...
  - 여기서 중요한 것은 자원을 분할해야 동시성을 보장할 수 있음. `FDM` 방식과 `TDM`방식이 있다. Frequency vs Time
  - `자원의 예약` 특성 때문에 **연속적** 연결인 전화 네트워크에서는 적합하지만 인터넷(데이터) 네트워크는 **비연속적** 연결이기 때문에 비적합히다. **다른 사용자가 자원을 잡을 수 없다.**
- Packet Switching(패킷 교환 방식)
  - 필요할 때 쓰게끔 한다.
  - 가용 자원이 없다면 먼저 온 transmission이 끝나기를 기다린다.
  - 한 연결이 한 링크를 잡으면 full link capacity를 사용해 각 패킷이 전달된다.
  - packet을 `chunk`로 나누어 보냄
  - `목적지 주소 명시` 필수
  - 메시지는 너무 길기 때문에 `data domination`을 방지하기 위해 패킷으로 나누는데, 이 패킷에는 **목적지 주소**가 담겨 있다. 이게너무 길면 `store`을 한다. 그래서 라우터는 이 패킷을 전부 다 받은 뒤 그 안에 담긴 주소로 다시 forward하게 된다(`store-and-forward`).
  - 자원을 reserve하지 않기때문에  **Queuing delay**가 발생할 수 있다. 도착 속도가 전송 속도보다 높으면 버퍼에 쌓이게 된다. 만약 버퍼가 꽉 차게 된다면 **packet loss**가 발생할 수 있다. 이 상황을 **Congestion**이라 한다.

##### Packet switching Vs. Circular switching
- Packet switching이 효율적, `control overhead`가 없음
- Packet switching은 `congestion` 때문에 `streaming data`와 같은 `audio/video`들에 대한 부작용
